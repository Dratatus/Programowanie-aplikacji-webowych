      ...
      <div class="container mx-auto max-w-7xl">
        ...
        <div v-if="detailedTask" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div class="bg-white dark:bg-gray-800 p-10 rounded-lg shadow-md max-w-md mx-auto">
            ...
            <div>
              ...
              <p class="text-gray-600 dark:text-gray-400">Description: {{ detailedTask.description }}</p>
              <p class="text-gray-600 dark:text-gray-400">Priority: {{ detailedTask.priority }}</p>
              <p v-if="detailedTask.assignedUserId" class="text-gray-600 dark:text-gray-400">Assigned to: {{ getUserName(detailedTask.assignedUserId) }}</p>
              <p class="text-gray-600 dark:text-gray-400">Estimated Hours: {{ detailedTask.estimatedTime }}</p>
              <p v-if="detailedTask.startDate" class="text-gray-600 dark:text-gray-400">Start Date: {{ formatDate(detailedTask.startDate) }}</p>
              <p v-if="detailedTask.endDate" class="text-gray-600 dark:text-gray-400">End Date: {{ formatDate(detailedTask.endDate) }}</p>
            </div>
        ...
        <div class="flex flex-row justify-between space-x-4">
          <div class="w-full lg:w-1/3 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            <h2 class="text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100">To Do Tasks</h2>
            <ul class="space-y-3">
              <li v-for="task in todoTasks" :key="task.id"
                ...
                <div>
                  ...
                  <p class="text-gray-600 dark:text-gray-400">Priority: {{ task.priority }}</p>
                  <p v-if="task.assignedUserId" class="text-gray-600 dark:text-gray-400">Assigned to: {{ getUserName(task.assignedUserId) }}</p>
                </div>
              ...
              <li v-if="todoTasks.length === 0" class="text-center text-gray-500 dark:text-gray-400">No tasks found.</li>
            </ul>
          </div>
  
          <div class="w-full lg:w-1/3 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            <h2 class="text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Doing Tasks</h2>
            <ul class="space-y-3">
              <li v-for="task in doingTasks" :key="task.id"
                ...
                <div>
                  ...
                  <p class="text-gray-600 dark:text-gray-400">Priority: {{ task.priority }}</p>
                  <p v-if="task.assignedUserId" class="text-gray-600 dark:text-gray-400">Assigned to: {{ getUserName(task.assignedUserId) }}</p>
                  <p class="text-gray-600 dark:text-gray-400">Estimated Hours: {{ task.estimatedTime }}</p>
                  <p v-if="task.startDate" class="text-gray-600 dark:text-gray-400">Start Date: {{ formatDate(task.startDate) }}</p>
                </div>
                <div class="flex flex-col items-center justify-between">
                  <div class="flex mb-2">
                    <button @click="editTask(task)"
              ...
              <li v-if="doingTasks.length === 0" class="text-center text-gray-500 dark:text-gray-400">No tasks found.</li>
            </ul>
          </div>
  
          <div class="w-full lg:w-1/3 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            <h2 class="text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Done Tasks</h2>
            <ul class="space-y-3">
              <li v-for="task in doneTasks" :key="task.id"
                ...
                <div>
                  ...
                  <p class="text-gray-600 dark:text-gray-400">Estimated Hours: {{ task.estimatedTime }}</p>
                  <p v-if="task.startDate" class="text-gray-600 dark:text-gray-400">Start Date: {{ formatDate(task.startDate) }}</p>
                  <p v-if="task.endDate" class="text-gray-600 dark:text-gray-400">End Date: {{ formatDate(task.endDate) }}</p>
                </div>
                <div class="flex flex-col items-center justify-between">
                  <div class="flex mb-2">
                    <button @click="editTask(task)"
              ...
              <li v-if="doneTasks.length === 0" class="text-center text-gray-500 dark:text-gray-400">No tasks found.</li>
            </ul>
          </div>
        </div>
      </div>
